...
export const useJournalEntry = (existingEntry?: { id: string; content: string }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (reflectionsResponse?.reflections) {
        ...
        // Prepare reflections for insertion with proper types
        ...
        // Type assertion to ensure reflections have the correct type
        if (savedReflections) {
          const typedReflections = savedReflections.map(r => ({
            ...r,
            type: r.type as 'celebration' | 'warning' | 'nudge'
          }));
          setReflections(typedReflections);
